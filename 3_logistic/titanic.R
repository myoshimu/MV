...
避ける
head(d.raw, 50) #はじめの50行表示
str(d.raw) #220M1レコードあることがわかる

#トレーニングデータとテストデータを分ける
index <- sample(1:2201,1500) #日復元抽出で1:2201から一様乱数
train <- d.raw[index,]
test <- d.raw[-index,]
head(index)
head(train)
...
for(i in 1:1000){
  ...
  train <- d.raw[index,]
  test <- d.raw[-index,]
  lrg.tit <- glm(Survived~Class+Sex+Age, data=train,family=binomial)
  p <- predict(lrg.tit, test, type="response")
  yp <- ifelse(p < 0.5, "No", "Yes")
  t <- table(test$Survived, yp)
