...
#MCMC
stantrend <- stan_model(model_code=trendModelN)
...
#信頼上限加限
upr <- exp(alpha + 1.96*summary(fit_trend)$summary[1:30,3])
lwr <- exp(alpha - 1.96*summary(fit_trend)$summary[1:30,3])

#描画, 対数正気なのでμが大きくなると分散も大きくなる
res <- data.frame(Y, exp(alpha), upr, lwr)
